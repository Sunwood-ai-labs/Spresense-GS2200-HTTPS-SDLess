...
typedef enum { POST = 0, GET } DEMO_STATUS_E;
...
void setup() {
  ...
  theHttpGs2200.config(HTTP_HEADER_CONTENT_TYPE,
                       "application/x-www-form-urlencoded");
  theHttpGs2200.config(HTTP_HEADER_HOST, HTTP_SRVR_IP);

  /* Set certificate from flash memory (no SD card needed) */
  /* AT+SETTIME format is DD/MM/YYYY,HH:MM:SS */
  char time_string[] = "18/02/2026,18:05:00";

  /* Delete existing cert first (if any) to avoid "FILE EXISTS" error */
  AtCmd_SendCommand((char *)"AT+TCERTDEL=TLS_CA\r\n");

  theHttpGs2200.set_cert((char *)"TLS_CA", time_string, 0, /* format: 0=PEM */
                         1,                                /* location: 1=RAM */
                         (uint8_t *)AMAZON_ROOT_CA1, strlen(AMAZON_ROOT_CA1));
